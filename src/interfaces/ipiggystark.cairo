use piggystark::structs::piggystructs::Target;
...
pub trait IPiggyStark<TContractState> {
    ...
    /// Funds are moved back to the user’s flexible balance and withdrawn from Nostra if
    /// applicable.
    ...
    /// Allows users to save toward a specific financial goal, with funds optionally allocated to
    /// Nostra.
    ...
    fn create_target(ref self: TContractState, goal: u256, deadline: u64) -> u64;
    ...
    fn contribute_to_target(ref self: TContractState, target_id: u64, amount: u256);

    /// Gets target count for contract
    ///  @return u64 target count for contract
    fn get_target_count(self: @TContractState) -> u64;

    /// Gets a target savings from an id
    /// Allows users to see details about a current target
    /// @param target_id the id of the target that is to be gotten
    /// @return Target struct of the id
    fn get_target(self: @TContractState, target_id: u64) -> Option<Target>;
